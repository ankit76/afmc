...
def initialize_walker(system_data):
    ...
    images = box.get_particle_images(pos)
    displacements = vmap(
        lambda x: x[:, jnp.newaxis, :] - pos[jnp.newaxis, :, :], in_axes=0
    )(images)
    distances = vmap(lambda x: jnp.linalg.norm(x))(
        displacements.reshape(-1, pos.shape[-1])
    ).reshape(images.shape[0], n_particles, n_particles)
    min_distances = vmap(lambda x: jnp.min(x))(
        distances.reshape(images.shape[0], -1).T
    ...
    energies = vmap(pot_fun)(min_distances.reshape(-1)).reshape(min_distances.shape)
    ...
    walker_data["prop_energy_change"] = 0.0
    ...
    walker_data["rdf_hist"] = (
        jnp.histogram(
            min_distances[jnp.triu_indices(n_particles, k=1)],
            system_data.n_bins,
            (0.0, system_data.r_max),
        )[0]
...
def prop_update_walker(walker_data, move, system_data):
    ...
    distances = vmap(lambda x: jnp.linalg.norm(x))(
        displacements.reshape(-1, pos.shape[-1])
    ).reshape(images.shape[0], n_particles)
    min_distances = vmap(lambda x: jnp.min(x))(
        distances.reshape(images.shape[0], -1).T
    ).reshape(n_particles)
    energies = vmap(pot_fun)(min_distances)
    ...
    rdf_hist_old = jnp.histogram(
        walker_data["distances"][move[0]], system_data.n_bins, (0.0, system_data.r_max)
    )[0]
    rdf_hist_new = jnp.histogram(
        min_distances, system_data.n_bins, (0.0, system_data.r_max)
    )[0]
    walker_data["prop_rdf_hist_change"] = (
        rdf_hist_new - rdf_hist_old
    ) / system_data.n_particles
    return walker_data
...
def metropolis_step(walker_data, move, random_num, system_data):
    ...
    new_pos = lax.cond(
        b_ratio > random_num,
        lambda x: walker_data["prop_pos"],
        lambda x: walker_data["pos"],
        0,
    )
    accepted = b_ratio > random_num
    new_energies = lax.cond(
        b_ratio > random_num,
        lambda x: x.at[move[0]].set(walker_data["prop_energies"]),
        lambda x: x,
        walker_data["energies"],
    )
    new_energies = lax.cond(
        b_ratio > random_num,
        lambda x: x.at[:, move[0]].set(walker_data["prop_energies"]),
    ...
    new_distances = lax.cond(
        b_ratio > random_num,
        lambda x: x.at[move[0]].set(walker_data["prop_distances"]),
        lambda x: x,
        walker_data["distances"],
    )
    new_distances = lax.cond(
        b_ratio > random_num,
        lambda x: x.at[:, move[0]].set(walker_data["prop_distances"]),
...
def sampling(system_data, random_data):
    ...
    # jax.debug.print('walker_data:\n{}\n', walker_data)
    walker_data, samples = lax.scan(
        scanned_fun, walker_data, jnp.arange(random_numbers.shape[0])
    )
    return (
        samples,
        walker_data["rdf_hist_avg"],
        walker_data["n_accepted"] / random_numbers.shape[0],
    )
...
def driver(
    ...
    n_dim = len(box.length)
    if n_bins is None:
        n_bins = 50
    if r_max is None:
        r_max = 5 * box.size / n_particles
    system_data = system(
        beta=beta,
        pot_fun=pot_fun,
        box=box,
    ...
    key, random_key = random.split(key)
    random_proposals = step_size * random.normal(random_key, shape=(n_samples, n_dim))
    key, random_key = random.split(key)
    random_indices = random.randint(
        random_key, shape=(n_samples,), minval=0, maxval=n_particles
    )
    ...
    comm.Reduce(
        [samples[1], MPI.DOUBLE], [global_energies, MPI.DOUBLE], op=MPI.SUM, root=0
    )
    ...
    if rank == 0:
        ...
        stat_utils.blocking_analysis(
            None, global_energies / n_particles, neql=n_eql, printQ=True
        )
        print(f"Acceptance ratio: {accepted}")

        # fourier transform to calculate structure factor from rdf
        sf = np.fft.fft(global_rdf)
        np.savetxt("sf.dat", sf.real)
        r = np.linspace(0.0, r_max, n_bins)
        np.savetxt("rdf_avg_p.dat", np.stack((r, global_rdf / (r_max / n_bins))).T)
...
if __name__ == "__main__":
    ...
    box = boxes.one_dimensional_box(50.0)
    pot_fun = lambda x: jnp.exp(-jnp.abs(x) ** 2)
    # pot_fun = lambda x: jnp.abs(x)**2
    n_particles = 50
    n_samples = 400000
    beta = 4.0
    samples, rdf_avg, accepted = driver(
        beta, n_particles, box, pot_fun, n_samples=n_samples, step_size=1.0, seed=1
    )
